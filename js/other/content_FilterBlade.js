(function() {
    ...
    console.log('[POE2市集汉化] FilterBlade 汉化脚本已加载');
    ...
    async function loadDictionary() {
        try {
            ...
            console.error('[FilterBlade] 加载词典失败:', err);
        }
    }

    chrome.storage.local.get(['language', 'siteLanguageSettings', 'siteEnabledSettings'], function(result) {
        if (result.siteEnabledSettings?.filterblade === false) return;
        
        const siteLang = result.siteLanguageSettings?.filterblade;
